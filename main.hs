...
checksum "--version" = do
    S.putStrLn "cksum (coreutils) 8.30\n\
...
cksum xs n i = if xs == BS.empty
    then (complement (func n i)) .&. 0xFFFFFFFF
    else cksum (BS.tail xs) (crc(BS.head xs) n) (i+1)

func :: Word32 -> Integer -> Word32
func n 0 = n
func n len = func (crc (fromInteger len) n) (len `shiftR` 8)

crc :: Word8 -> Word32 -> Word32
crc x n = (n `unsafeShiftL` 8) `xor` crctab!!(fromInteger (toInteger (((n `unsafeShiftR` 24) `xor` (fromInteger (toInteger x))) .&. 0xFF)))
